...
describe('FilteredDocuments', () => {
  ...
  describe('searching', () => {
    ...
    it('shows the matching scenario when a step matches', async () => {
      const { getByRole, getByText, queryByRole } = render(
        <EnvelopesProvider envelopes={retry}>
          <InMemorySearchProvider defaultQuery="third">
            <FilteredDocuments />
          </InMemorySearchProvider>
        </EnvelopesProvider>
      )
      ...
      expect(getByText('a step that passes the third time')).to.be.visible
      ...
      expect(
        queryByRole('heading', {
          name: "Scenario: Test cases won't retry after failing more than the --retry limit",
        })
      ).not.to.exist
    })

    it('shows the matching scenario when a scenario name matches', async () => {
      const { getByRole, queryByRole } = render(
        <EnvelopesProvider envelopes={attachments}>
      ...
      await waitFor(() => getByRole('heading', { name: 'Scenario: Log JSON' }))

      expect(queryByRole('heading', { name: 'Scenario: Log text' })).not.to.exist
      expect(queryByRole('heading', { name: 'Scenario: Log ANSI coloured text' })).not.to.exist
    })

    it('shows the matching rule and everything under it', async () => {
      const { getByRole, queryByRole } = render(
        <EnvelopesProvider envelopes={rules}>
          <InMemorySearchProvider defaultQuery="enough money">
            <FilteredDocuments />
          </InMemorySearchProvider>
        </EnvelopesProvider>
      )
      ...
      expect(getByRole('heading', { name: 'Example: Enough money' })).to.be.visible
      expect(getByRole('heading', { name: 'Example: Not enough money' })).to.be.visible
      expect(queryByRole('heading', { name: 'Example: No chocolates left' })).not.to.exist
    })

    it('shows everything under the parent when a background matches', async () => {
      const { getByRole } = render(
        <EnvelopesProvider envelopes={backgrounds}>
          <InMemorySearchProvider defaultQuery="eggs">
            <FilteredDocuments />
          </InMemorySearchProvider>
        </EnvelopesProvider>
      )

      await waitFor(() => getByRole('button', { name: 'samples/backgrounds/backgrounds.feature' }))
      await userEvent.click(
        getByRole('button', { name: 'samples/backgrounds/backgrounds.feature' })
      )

      expect(getByRole('heading', { name: 'Scenario: one scenario' })).to.be.visible
      expect(getByRole('heading', { name: 'Scenario: another scenario' })).to.be.visible
    })

    it('shows everything when a feature matches', async () => {
      const { getByRole } = render(
        <EnvelopesProvider envelopes={rules}>
          <InMemorySearchProvider defaultQuery="synonym">
            <FilteredDocuments />
          </InMemorySearchProvider>
        </EnvelopesProvider>
      )
  ...
  describe('filtering by status', () => {
    ...
    it('hides examples table heading when all its examples are filtered out', async () => {
      const { getByRole, queryByRole } = render(
        <EnvelopesProvider envelopes={examplesTables}>
          <InMemorySearchProvider defaultHideStatuses={[TestStepResultStatus.FAILED]}>
            <FilteredDocuments />
      ...
      // examples table with passing examples should be visible
      expect(getByRole('heading', { name: 'Examples: These are passing' })).to.be.visible
      // examples table where all examples are filtered out should not show its heading
      expect(queryByRole('heading', { name: 'Examples: These are failing' })).not.to.exist
    })
