    ...
    return useMemo(() => {
        ...
        for (const testCaseFinished of cucumberQuery.findAllTestCaseFinished()) {
            const testCaseStarted =
                cucumberQuery.findTestCaseStartedBy(testCaseFinished);
            if (!testCaseStarted) {
                continue;
            }
            ...
            const status =
                cucumberQuery.findMostSevereTestStepResultBy(testCaseFinished)
            ...
            const feature =
                cucumberQuery.findLineageBy(testCaseStarted)?.feature?.name ??
                "";
            ...
            items.push({
                ...
                tags: pickle.tags,
                status,
                start: TimeConversion.timestampToMillisecondsSinceEpoch(
                    testCaseStarted.timestamp,
                ),
                end: TimeConversion.timestampToMillisecondsSinceEpoch(
        ...
        const start =
            items.length > 0 ? Math.min(...items.map((item) => item.start)) : 0;
        const end =
            items.length > 0 ? Math.max(...items.map((item) => item.end)) : 0;
